    ...
    <el-table ref="multipleTable" :data="formData" style="width: 100%" stripe @select="handleSelectionChange">
      ...
      <el-table-column prop="type" label="任务类型">
        <template slot-scope="scope">
          {{scope.row.type==1?"设备运维任务":scope.row.type==2?"手动运维计划":scope.row.type==3?"报修工单":scope.row.type==4?"巡查计划":scope.row.type==5?'报警处理':'其它类型'}}
        </template>
      </el-table-column>
      ...
      <div class="cont_box_left">
        <el-table :data="formData3" style="width: 100% height:300px;">
          <el-table-column prop="" label="编号">
              <template slot-scope="scope">
                <el-radio @change="baojingName=scope.row.nickname" v-model="alarmId" :label="scope.row.id">{{scope.row.id}}</el-radio>
              </template>
          </el-table-column>
        ...
        <el-button size="small" type="primary" @click="baojingDialog=false" style="float:right;margin-top:20px;">关 闭</el-button>
...
export default {
  ...
  methods:{
    ...
    addList(addList){//添加
      if(this.alarmId){
        this.formPush.alarmId=this.alarmId
      }
    ...
    updateList(formUpdate){//修改
      if(this.alarmId){
        this.formUpdate.alarmId=this.alarmId
      }

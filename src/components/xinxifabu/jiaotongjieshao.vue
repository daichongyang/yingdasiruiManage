<template>
  ...
  <el-table-column prop="type" label="类型">
    <template slot-scope="scope">
  		{{scope.row.type==1?"公共交通线路":scope.row.type==2?"自驾路线":"图片带详情介绍"}}
  	</template>
  </el-table-column>
        ...
        <el-form label-position="right" label-width="110px" :model="formPush" :rules="rules"  ref='addList'>
         <el-form-item label="标题" prop="title">
          ...
          <el-form-item label="类型" size="small">
            <el-select v-model="formPush.type" placeholder="请选择出行类型" prop="advertType">
              <el-option label="公共交通线路" :value="1"></el-option>
              <el-option label="自驾路线" :value="2"></el-option>
            </el-select>
          </el-form-item>
          <div v-for="(item,index) in nodeList" :key="index">
            ...
            <el-form-item label="自定义" size="small">
              <el-button size="medium " @click="addCont">添 加</el-button>
            </el-form-item>
        </el-form>
      </div>
 ...
 <el-form-item label="公园id">
     ...
     <el-form-item label="创建时间" >
        ...
                </el-date-picker>
          ...
          <el-form-item label="类型" size="small">
            <el-input v-model="formUpdate.type==1?'公共交通线路':formUpdate.type==2?'自驾路线':''"></el-input>
          </el-form-item>
    <div v-for="(item,index) in formUpdate.nodeList" :key="index">
            <el-form-item label="路线标题" size="small" >
              <el-input v-model="item.title"></el-input>
            </el-form-item>
            <el-form-item label="内容" size="small">
              <el-input v-model="item.content"></el-input>
...
import { getTraffic,addTraffic,delTraffic } from '../../url/api';
export default {
  data(){
    return{
      imageUrl:'',
  ...
  handleAvatarSuccess(res, file) {
    ...
    handleAvatarSuccess1(res, file) {
      console.log(res, file)
      this.pictrueNum++;
      this.formPush.coverVideo = res.data[0]+","+this.formPush.coverVideo
    ...
    getlist(){
   getTraffic(this.formSearch).then((res)=>{
     console.log(res)
     if(res.data.code == 200){
    ...
    addList(addList){//添加
      this.formPush.nodeList=[]
      this.nodeList.forEach(item=>{
        this.formPush.nodeList.push(item)
      })
      this.$refs[addList].validate((valid) => {
        if (valid) {

          addTraffic(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({

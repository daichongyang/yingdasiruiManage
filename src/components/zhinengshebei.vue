...
import { Facilitieslist,Facilitiessave,Facilitiesupdate,Facilitiesdelect } from '../url/api.js';
export default {
  data(){
    return{
      imageUrl:'',
   ...
   handleAvatarSuccess3(res, file) {
    ...
    addList(addList){//添加
      this.$refs[addList].validate((valid) => {
        if (valid) {
          Facilitiessave(this.formPush).then((res)=>{
            console.log(res)
            if(res.data.code == 200){
              this.getlist()
              this.$message({
    ...
    deleInfor(ids){//删除
      ...
      this.$confirm('确认删除吗？')
      .then(_ => {
        Facilitiesdelect(arrId).then((res)=>{
          console.log(res)
          if(res.data.code == 200){
            this.$message('删除成功');
            this.getInit()
